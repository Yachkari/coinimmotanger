...
export default function ContactForm({ listing }: Props) {
  ...
  const defaultMessage = lang === 'fr'
    ...
    : `Hello, I am interested in: "${listing.title_en ?? listing.title}". Please contact me.`
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) { setError(data.error ?? data.errors?.[0] ?? tr(t.contact.errorBody, lang)); return; }
      setSuccess(true);
    } catch {
  ...
  if (success) {
    return (
      <div className="cf cf--success">
        ...
        <h3 className="cf__success-title">{tr(t.contact.successTitle, lang)}</h3>
        <p className="cf__success-text">{tr(t.contact.successBody, lang)}</p>
    ...
    <div className="cf">
      ...
      <div className="cf__preview">
        <div className="cf__preview-info">
          <span className="cf__preview-label">
            {lang === 'fr' ? 'Votre demande concerne' : 'Your enquiry is about'}
          </span>
          <span className="cf__preview-title">
            {lang === 'en' ? (listing.title_en ?? listing.title) : listing.title}
          </span>
          <span className="cf__preview-price">
      ...
      <div className="cf__divider">
        <span>{lang === 'fr' ? 'ou envoyer un email' : 'or send an email'}</span>
      </div>
      ...
      <form onSubmit={handleSubmit} className="cf__form">
        <div className="cf__row">
          <div className="cf__field">
            <label className="cf__label">{tr(t.contact.name, lang)} *</label>
            <input
              className="cf__input" value={name}
              onChange={e => setName(e.target.value)}
              placeholder={tr(t.contact.namePlaceholder, lang)} required
            />
          </div>
          <div className="cf__field">
            <label className="cf__label">{tr(t.contact.phone, lang)}</label>
            <input
              type="tel" className="cf__input" value={phone}
        ...
        <div className="cf__field">
          <label className="cf__label">{tr(t.contact.email, lang)} *</label>
          <input
            type="email" className="cf__input" value={email}
        ...
        <div className="cf__field">
          <label className="cf__label">{tr(t.contact.message, lang)} *</label>
          <textarea
            className="cf__input" value={message}
        ...
        <button type="submit" className="cf__submit" disabled={loading}>
          {loading
            ? <span className="cf__spinner" />
            : <><Send size={14} /> {tr(t.contact.send, lang)}</>
          }
        </button>
      </form>
